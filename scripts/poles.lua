...
function on_pole_built(pole)
    ...
    for _, neighbour in pairs(pole.neighbours.copper) do
        if neighbour.type == "electric-pole" and
            (pole_name == "po-hidden-electric-pole-in" or pole_name == "po-hidden-electric-pole-out" or neighbour.name ==
                "po-hidden-electric-pole-in" or neighbour.name == "po-hidden-electric-pole-out" or
    ...
    for _ = 1, poles_to_check do
        ...
        if pole and pole.valid then
            ...
            if max_consumption and consumption > max_consumption then
                local rando = math.random() * (consumption / max_consumption)
                log(rando)

                if destroy_pole_setting == "destroy" then
                    log("Pole being killed at consumption " .. math.ceil(consumption / 1000000) ..
                            "MW which is above max_consumption " .. math.ceil(max_consumption / 1000000) .. "MW")
